...
public class GraphSearch extends Configured implements Tool {
  ...
  public static class MapClass extends MapReduceBase implements
    ...
    // input key in mapper is node id and input value is sth like  2,6|1,10|0|GRAY  	
    public void map(LongWritable key, Text value, OutputCollector<IntWritable, Text> output,
      ...
      if (node.getColor() == Node.Color.GRAY) {
        ...
        for (int v : node.getEdges()) { // go through each edge node such as 2,6 
          Node vnode = new Node(v);
          vnode.setDistance(node.getDistance() + node.getWeights().get(index));
  ...
  public static class Reduce extends MapReduceBase implements
     ...
     * - The darkest Color
     */
    // input key: node id, input value 2,6|1,10|0|GRAY
    public void reduce(IntWritable key, Iterator<Text> values,
      ...
      while (values.hasNext()) {
        ...
        // creat a Node u to store 
        Node u = new Node(key.get() + "  " + value.toString());
        //System.out.println("id: " + key.toString() + " edges: " )
        ...
        // Save the minimum distanceordinal
  ...
  public int run(String[] args) throws Exception {
    ...
    while (keepGoing(iterationCount, args)) {
  ...
  private boolean keepGoing(int iterationCount, String[] args) {
    int count = 0;
    for(int i = 0; i < args.length; ++i) {
      if ("-i".equals(args[i])) {
